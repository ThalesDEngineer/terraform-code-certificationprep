# Setting refresh false along with target flags
# terraform plan -refresh=false -target=aws_security_group.allow_ssh_conn
# target flag allows you to only run api calls to specific(s) resource(s)
# refresh=false prevents refreshing the state for every resource
